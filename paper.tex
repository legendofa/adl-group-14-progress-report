...
\icmlauthor{Julian Felix Kieslinger 2025-82736}{ds}
\icmlauthor{Salah-Eddine Nifa 2025-82914}{ds}
\icmlauthor{Jonghyun Shin 2024-30227}{ece}
\icmlauthor{Wun Hur 2023-26041}{ipai}
%\icmlauthor{}{sch}
%\icmlauthor{}{sch}
\end{icmlauthorlist}

\icmlaffiliation{ds}{Department of Data Science, Seoul National University, Seoul, SK}
\icmlaffiliation{ece}{Department of Electrical and Computer Engineering, Seoul National University, Seoul, South Korea}
\icmlaffiliation{ipai}{Interdisciplinary Program in Artificial Intelligence, Seoul National University, Seoul, South Korea}

\icmlcorrespondingauthor{Julian Felix Kieslinger 2025-82736}{kieslijn@snu.ac.kr}
\icmlcorrespondingauthor{Salah-Eddine Nifa 2025-82914}{salah03@snu.ac.kr}
\icmlcorrespondingauthor{Jonghyun Shin 2024-30227}{jonghyun\_shin@snu.ac.kr}
\icmlcorrespondingauthor{Wun Hur 2023-26041}{hurwun0103@snu.ac.kr}
...
Another complementary research direction gives LLMs explicit control over their own memory. MemGPT introduced an operating-system–inspired virtual memory manager that partitions information into “main” and “external” memory and allows the model to swap relevant context dynamically during conversation \cite{memgpt2023}. This enables scalable long-context reasoning without exceeding token limits. Similarly, persistent-memory frameworks in OpenAI’s ChatGPT Memory Update (2024) \cite{openai_memory2024} store user-specific information across sessions, marking a commercial step toward continuous, personalized agents.

To evaluate such systems, benchmarks like LongMemEval \cite{longmemeval2024} were proposed to assess an agent’s long-term factual retention, consistency, and adaptability over multi-session dialogues averaging more than 100K tokens of cumulative context. Systems employing hybrid knowledge–semantic memory, such as Graphiti, have achieved higher relevance and accuracy scores on LongMemEval by retrieving structured facts rather than full histories.

In summary, existing approaches, including GraphRAG, Graphiti, MemGPT, and persistent OpenAI memory, have significantly advanced LLM memory architectures. Yet none fully integrate interpretable structured knowledge with scalable unstructured semantic recall under autonomous update control. Our proposed framework bridges this gap by combining a knowledge graph for explicit entity relations with a vector database for flexible semantic recall, enabling a persistent, adaptive, and context-aware memory foundation for next-generation agentic LLMs.
...
\subsection{Zep Baseline Construction}\label{sec:zep-baseline-construction}
...
Since our goal is to build an LLM agent with long-term memory,
we have been working on the integration of necessary components for our agent,
    reproducing the baseline model Zep (of which the source code is not publicly available),
    and building an evaluation pipeline for our agent.

\subsection{Integration of Components}
We built an environment with several components to implement the memory-augmented agentic LLM,
including the following components:
\begin{enumerate}
    \item \texttt{n8n}: a GUI-based tool which allows to build a workflow with LLMs and other tools.
    \item \texttt{Ollama}: a lightweight LLM server which is used to run LLMs locally.
    \item \texttt{Open WebUI}: a self-hosted AI interface for interacting with the agent.
    \item \texttt{PostgreSQL}: a relational database which will serve as a vector database for RAG.
    \item \texttt{Graphiti}: the core component of Zep, which is published as an open-source project.
\end{enumerate}
We tested the integration with a simple workflow.

\subsection{Implementation of Zep Baseline}
As shown in Figure~\ref{fig:zep-extraction-workflow}, we started implementing the Zep-like baseline
by concretizing the extraction workflow from the paper.
To meet the time constraints, we seek to simplify the implementation
by maximizing the utilization of Graphiti and taking the community node out of the scope as stated in
Section~\ref{sec:zep-baseline-construction}.
...
Our next steps include completely implementing the Zep baseline, developing and integrating our proposed method, and conducting comprehensive evaluations using the LongMemEval benchmark.
